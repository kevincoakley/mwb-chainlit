...
async def on_message(message: cl.Message):
    ...
    # Build conversation context with system prompt and user message
    ...
    if tool_calls:
        # Build conversation history including the assistant's tool call requests
        ...
        second = await client.chat.completions.create(
            messages=follow_messages + tool_results_msgs,
            **settings,
        )
        await cl.Message(content=second.choices[0].message.content or "").send()
        return

    # No tool calls were made - send the model's direct response
